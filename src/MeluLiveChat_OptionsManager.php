...
class MeluLiveChat_OptionsManager
{
    ...
    public function verifyConnection()
    {
        if ($this->_isCurl()) {
            ...
            $curl = new Curl();
            $curl->setReferer($host);
            $curl->setOpt(CURLOPT_FOLLOWLOCATION, true);
            $curl->get("https://meluchat.com/livechat/script.php?id=" . $this->getOption("key"));
            if ($curl->error) {
                echo 'Error: ' . $curl->errorCode . ': ' . $curl->errorMessage . "\n";
                $active = 'Sorry we could not validate your key, this is probably due to server configuration, please check the frontend to see if it is working.';
            } else {
            ...
            return 'Your server does not allow for us to check the functionality of your live chat. For this check to work please enable php_curl';
        }
    }
     ...
     */
    public
    function addOption($optionName, $value)
    {
        $prefixedOptionName = $this->prefix($optionName); // how it is stored in DB
     ...
     */
    public
    function getRoleOption($optionName)
    {
     ...
     */
    protected
    function roleToCapability($roleName)
    {
        switch ($roleName) {
            case 'Super Admin':
     ...
     */
    public
    function isUserRoleEqualOrBetterThan($roleName)
    {
        if ('Anyone' == $roleName) {
            return true;
        }
     ...
     */
    public
    function canUserDoRoleOption($optionName)
    {
    ...
    function settingsPage()
    {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'melu-live-chat'));
        }
        ...
        <div class="wrap">
            ...
            if (version_compare('5.6', phpversion()) > 0) {
                ...
                _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.6)', 'melu-live-chat');
                echo '</span>';
            }

            /** Uncomment below if use of DB Functions */
            /*if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'melu-live-chat');
                echo '</span>';
            }*/
     ...
     */
    protected
    function createFormControl($aOptionKey, $aOptionMeta, $savedOptionValue)
    {
     ...
     */
    protected
    function getOptionValueI18nString($optionValue)
    {
        switch ($optionValue) {
            case 'true':
                return __('true', 'melu-live-chat');
            case 'false':
                return __('false', 'melu-live-chat');

            case 'Administrator':
                return __('Administrator', 'melu-live-chat');
            case 'Editor':
                return __('Editor', 'melu-live-chat');
            case 'Author':
                return __('Author', 'melu-live-chat');
            case 'Contributor':
                return __('Contributor', 'melu-live-chat');
            case 'Subscriber':
                return __('Subscriber', 'melu-live-chat');
            case 'Anyone':
                return __('Anyone', 'melu-live-chat');
        }
        return $optionValue;
    }
